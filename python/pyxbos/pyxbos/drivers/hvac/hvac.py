...
class HVACDriver(Driver):
    def setup(self, cfg):
        ...
        self.point_class_map = cfg["point_class_map"]
    ...
    def read(self, requestid=None):
        for dev in self.devices:
            ...
            output = {}
            for equipclass in point_class_map:
                if equipclass not in output:
                    output[equipclass] = {}

                for point in point_class_map[equipclass]:
                    pieces = point.split(".")
                    new_point = point
                    if len(pieces) > 2:
                    ...
                    if new_point in point_value_map:
                        pt_class = point_class_map[equipclass][point]['point_class']
...
if __name__ == '__main__':
    ...
    point_class_map = get_point_class_mapping(pymortar_client, building)

    current_driver = HVACDriver({ "devices": bacnet.devices, "bacnet": bacnet, "point_class_map": point_class_map })
